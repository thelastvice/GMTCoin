...
#if defined(NDEBUG)
# error "Gmtcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xac633d578ded0af15669bcd693aabe91f799dbd4b9bb5baf96a759b2a60e2b21");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Gmtcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Gmtcoin Signed Message:\n";
...
{
    // Gmtcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Gmtcoin: 10k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Gmtcoin: 1 days
static const int64 nTargetSpacing = 5 * 60; // Gmtcoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Gmtcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Gmtcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x79bbb205f9166cb3a5bfe31977f8e1399a428c42c483172d47facc34646716f9");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1624278081;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085743716;

        if (fTestNet)
        {
            block.nTime    = 1624278068;
            block.nNonce   = 386190868;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xdf2fe767f27a7afdea613d59ec275b354352ae9eeb584001fade2ec8c0a97d25"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Gmtcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GmtcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GmtcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GmtcoinMiner(CWallet *pwallet)
{
    printf("GmtcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("gmtcoin-miner");
    ...
    try { loop {
        ...
        printf("Running GmtcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GmtcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GmtcoinMiner, pwallet));
}
